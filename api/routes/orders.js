...
const query = { text: 'select o.id, o.idusr, o.total , o.total, o.fechasolicitada, o.fechaentrega, o.descripcion, o.idestado , e.nombre , e.nombre from orden o left join estado e on o.idestado = e.id left join establecimiento l on o.lugar = l.id where o.idusr = $1' ,
values: [userId] }
const response = res;
...
router.get("/lastOrder/:userId",function(req, res, next){
  ...
  const params = req.params
  const userId = params.userId;
  const query = { text: 'select id, total, descripcion, lugar, fechasolicitada, idestado from public.orden where idusr = $1 order by id desc', values: [userId] }
  const response = res;
  pool.query(query, (err, res) => {
    if(err) {
      response.send(err.stack)
